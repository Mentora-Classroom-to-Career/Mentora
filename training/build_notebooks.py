    ...
    cells = [
        ...
        *SHARED_SETUP,
        ...
        md("## 2b. Class-imbalance weighting — needed for a small, single-topic-per-question dataset\n\n"
           ...
           "`BCEWithLogitsLoss`) is the standard fix for this."),
        code(
            "import torch\n"
            "import torch.nn as nn\n\n"
        ...
        code(
            ...
            "                          # and see the threshold-sweep cell below for why 5 wasn't enough\n"
            "    learning_rate=2e-5,\n"
            "    save_strategy='epoch',\n"
            "    eval_strategy='epoch',\n"
        ...
        md("**Reading the result:**\n"
           "- If a lower threshold (e.g. 0.2-0.3) recovers a reasonable F1, the model learned "
           "the right ranking, it's just underconfident from too little data — this is fixable "
           "without more data by using that lower threshold in the app's inference code, though "
           "growing the question bank is still the real fix long-term.\n"
           "- If F1 stays near 0.0 even at threshold 0.1, the model hasn't learned useful "
           "signal at all yet — that's the 70-question starter set being genuinely too small "
           "(59 training rows / 16 classes averages under 4 examples per class). Scaling up "
           "M1's question bank (datasets/scripts/generate_m1_starter_bank.py, or better, real "
           "past-paper questions per datasets/SOURCES.md) is the fix, not more epochs or a "
           "different threshold."),
