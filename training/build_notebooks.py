    ...
    cells = [
        ...
        *SHARED_SETUP,
        ...
        md("## 4. Training arguments — memory-conscious for a free T4\n\n"
           "**`fp16` is off below.** A real run hit the exact textbook T5 fp16 "
           "failure: `eval_loss: nan` from early in training, training loss "
           "collapsing to `0.000000`, BLEU-4/ROUGE-L stuck near their random-"
           "init floor the whole run. This is a well-documented, still-"
           "unresolved HuggingFace issue (multiple open GitHub issues and forum "
           "threads) specific to **T5-Large and up** — `t5-small`/`t5-base` "
           "train fine in fp16, but `t5-large` and `flan-t5-xl` reliably "
           "produce inf/NaN in their feed-forward and attention layers under "
           "fp16's limited dynamic range. Our model (FLAN-T5-**Large**) is "
           "exactly the affected size. `bf16` would sidestep this on hardware "
           "with real bf16 tensor cores, but a free-tier T4 (Turing "
           "architecture) doesn't have those, so fp32 is the safer choice here "
           "— the same conclusion as M1's DeBERTa fix, for an unrelated "
           "reason. LoRA keeps trainable parameters to ~0.3% of the model, so "
           "fp32 activations/gradients should still fit a T4's 15GB "
           "comfortably; drop `per_device_train_batch_size` to 2 first if you "
           "hit an OOM.\n\n"
           "**`num_train_epochs` raised from 3 to 15, with early stopping "
           "added.** A real fp32 run completed 3 epochs cleanly (no more NaN) "
           "but landed at BLEU4=0.035 / ROUGEL=0.171 — both far under target, "
           "and importantly, validation loss was STILL decreasing at the final "
           "step (0.601 -> 0.599) and BLEU/ROUGE were still climbing. That's "
           "a model that hasn't finished converging, not a broken one — 3 "
           "epochs was simply too few for a from-scratch LoRA fine-tune on "
           "6,942 examples. `EarlyStoppingCallback` decides when to actually "
           "stop instead of cutting off arbitrarily."),
        code(
            ...
            "    num_train_epochs=15,  # raised from 3 -- val loss/BLEU/ROUGE were all still\n"
            "                          # improving at epoch 3, this wasn't a converged result\n"
            "    learning_rate=1e-4,\n"
            "    predict_with_generate=True,\n"
            "    fp16=False,  # see note above -- FLAN-T5-Large is well-documented to produce\n"
            "                  # NaN loss under fp16; a T4 has no real bf16 support either, so fp32\n"
            "    save_strategy='steps',\n"
            "    save_steps=200,\n"
            "    eval_strategy='steps',\n"
            "    eval_steps=200,\n"
            ...
            ")\n\n"
            "def find_valid_checkpoint(out_dir):\n"
            "    # Same guard as M1's notebook -- a prior run's checkpoints can be NaN-corrupted\n"
            "    # (this happened here: eval_loss was already nan at the FIRST save, step 200,\n"
            "    # so every checkpoint from that run is unusable). Sort numerically, skip anything\n"
            "    # missing required files, and never silently resume into a corrupted run.\n"
            "    candidates = glob.glob(f'{out_dir}/checkpoint-*')\n"
            "    candidates = sorted(candidates, key=lambda p: int(p.rsplit('-', 1)[-1]), reverse=True)\n"
            "    for path in candidates:\n"
            "        if os.path.exists(f'{path}/trainer_state.json') and (\n"
            "            os.path.exists(f'{path}/model.safetensors') or os.path.exists(f'{path}/pytorch_model.bin')\n"
            "            or os.path.exists(f'{path}/adapter_model.safetensors')  # LoRA-specific weight file name\n"
            "        ):\n"
            "            return path\n"
            "        print(f'Skipping incomplete checkpoint: {path}')\n"
            "    return None\n\n"
        ...
        md("## 6. Qualitative check — actually look at what it generates\n\n"
           "BLEU/ROUGE are aggregate scores; they don't tell you WHETHER the model "
           "learned the JSON structure at all versus just needing sharper wording. "
           "Since a large fraction of the target string is identical boilerplate "
           "across every example (the `{\"question\": ..., \"options\": {\"A\": ..., "
           "...}, \"correct_answer\": ...}` schema), a model that's still garbling "
           "the JSON structure itself scores very differently from one that's "
           "producing valid JSON with just slightly-off wording — and the fix for "
           "each is different (more epochs vs. a data/prompt issue)."),
        code(
            "import json\n\n"
            "sample = ds['test'].select(range(8))\n"
            "valid_json_count = 0\n"
            "for ex in sample:\n"
            "    input_text = tokenizer.decode([t for t in ex['input_ids'] if t != tokenizer.pad_token_id], skip_special_tokens=True)\n"
            ...
            "        valid_json_count += 1\n"
            "        print('-> valid JSON with all required keys')\n"
            "    except Exception as e:\n"
            "        print(f'-> INVALID: {e}')\n"
            "    print()\n\n"
            "print(f'{valid_json_count}/{len(sample)} generated valid, complete JSON')\n"
            "print('If this is low, the model has not yet learned the output structure --')\n"
            "print('more epochs is the right fix. If this is high but BLEU/ROUGE stay low,')\n"
            "print('the issue is wording quality, not structure -- a different problem.')"
        ),
        md("## Target metric: BLEU-4 >= 0.35, ROUGE-L >= 0.50\n\nIf it plateaus lower:\n"
           "- Check the §6 qualitative output first — valid-JSON rate tells you whether "
           "this is a structure problem or a wording problem, which need different fixes\n"
